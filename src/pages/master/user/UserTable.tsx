...
const HandleDelete = (id: number) => {
  try {
    api.delete(`/users/${id}`, { withCredentials: true }).then((res) => {
      if (res.data.success === true) {
        mutate("/users"); // 🔥 Auto-refresh tabel tanpa reload halaman
      }
    });
  } catch (error) {
...
const UserTable = ({ setEditData }) => {
  const { data: rowData, error, mutate } = useSWR("/users", fetcher);
