...
const HandleDelete = (id: number) => {
  try {
    api.delete(`/trucks/${id}`, { withCredentials: true }).then((res) => {
      if (res.data.success === true) {
        mutate("/trucks"); // 🔥 Auto-refresh tabel tanpa reload halaman
      }
    });
  } catch (error) {
...
const TruckTable = ({ setEditData }) => {
  const { data: rowData, error, mutate } = useSWR("/trucks", fetcher);
