...
import styles from "./HandlingTable.module.css";
...
const HandleDelete = (id: number) => {
  try {
    api.delete(`/handling/${id}`, { withCredentials: true }).then((res) => {
      if (res.data.success === true) {
        mutate("/handling"); // 🔥 Auto-refresh tabel tanpa reload halaman
      }
    });
  } catch (error) {
...
const HandlingTable = ({ setEditData }) => {
  const { data: rowData, error, mutate } = useSWR("/handling", fetcher);
