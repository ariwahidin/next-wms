...
export default function StockOpnamePage() {
  ...
  const handleSubmit = async () => {
    ...
    // Cek apakah barcode + lokasi ini sudah pernah discan sebelumnya
    const existing = dataStockTakeBarcode.filter(
      (item) =>
    ...
    if (existing.length > 0) {
      const totalExisting = existing.reduce((sum, item) => sum + item.counted_qty, 0);
      const confirmDuplicate = confirm(
        `Barcode "${barcode}" di lokasi "${location}" sudah pernah discan sebanyak ${totalExisting} pcs (${existing.length}x entry). Tetap tambahkan ${qty} pcs lagi?`
      );
