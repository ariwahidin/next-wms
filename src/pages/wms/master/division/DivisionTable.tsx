...
import styles from "./OriginTable.module.css"; // reuse CSS yang sama
...
const HandleDelete = (id: number) => {
  try {
    api.delete(`/divisions/${id}`, { withCredentials: true }).then((res) => {
...
const DivisionTable = ({ setEditData }) => {
  const { data: rowData } = useSWR("/divisions", fetcher);

  const [columnDefs] = useState<ColDef[]>([
    { field: "no", headerName: "No.", maxWidth: 70 },
    { field: "code", headerName: "Code", width: 110 },
    { field: "name", headerName: "Division Name", flex: 1 },
