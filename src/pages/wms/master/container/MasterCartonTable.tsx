...
import styles from "./MasterCartonTable.module.css";
...
const HandleDelete = (id: number) => {
    ...
    try {
        api.delete(`/master-cartons/${id}`, { withCredentials: true }).then((res) => {
            if (res.data.success === true) {
                mutate("/master-cartons");
            }
        });
    } catch (error) {
...
const MasterCartonTable = ({ setEditData }) => {
    const { data: rowData, error, mutate } = useSWR("/master-cartons", fetcher);
...
export default MasterCartonTable;