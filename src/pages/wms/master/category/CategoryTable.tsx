...
const HandleDelete = (id: number) => {
  try {
    api.delete(`/categories/${id}`, { withCredentials: true }).then((res) => {
      if (res.data.success === true) {
        mutate("/categories"); // 🔥 Auto-refresh tabel tanpa reload
      }
    });
  } catch (error) {
...
const CategoryTable = ({ setEditData }) => {
  const { data: rowData, error, mutate } = useSWR("/categories", fetcher);
