...
const HandleDelete = (id: number) => {
  try {
    api.delete(`/qa-status/${id}`, { withCredentials: true }).then((res) => {
      if (res.data.success === true) {
        mutate("/qa-status"); // 🔥 Auto-refresh tabel tanpa reload halaman
      }
    });
  } catch (error) {
...
const Table = ({ setEditData }) => {
  const { data: rowData, error, mutate } = useSWR("/qa-status", fetcher);

  const [columnDefs, setColumnDefs] = useState<ColDef[]>([
    { field: "no", headerName: "No. ", maxWidth: 70 },
    { field: "qa_status", headerName: "QA Status", width: 140 },
