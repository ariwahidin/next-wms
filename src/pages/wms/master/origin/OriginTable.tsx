...
import styles from "./OriginTable.module.css";
...
const HandleDelete = (id: number) => {
  try {
    api.delete(`/origins/${id}`, { withCredentials: true }).then((res) => {
      if (res.data.success === true) {
        mutate("/origins"); // 🔥 Auto-refresh tabel tanpa reload halaman
      }
    });
  } catch (error) {
...
const OriginTable = ({ setEditData }) => {
  const { data: rowData, error, mutate } = useSWR("/origins", fetcher);

  const [columnDefs, setColumnDefs] = useState<ColDef[]>([
    { field: "no", headerName: "No. ", maxWidth: 70 },
    { field: "country", headerName: "Origin Country", width: 140 },
    {
      headerName: "Actions",
      field: "ID",
      cellRenderer: (params) => {
