...
import styles from "./TransporterTable.module.css";
...
const HandleDelete = (id: number) => {
  try {
    api.delete(`/transporters/${id}`, { withCredentials: true }).then((res) => {
      if (res.data.success === true) {
        mutate("/transporters"); // 🔥 Auto-refresh tabel tanpa reload halaman
      }
    });
  } catch (error) {
...
const TransporterTable = ({ setEditData }) => {
  const { data: rowData, error, mutate } = useSWR("/transporters", fetcher);
  ...
  const handleExport = async () => {
    ...
    try {
      const response = await api.get("/transporters/export", {
      ...
      if (response.data.success) {
        ...
        const exportData = transporters.map((transporter, index) => ({
          No: index + 1,
          "Transporter Code": transporter.transporter_code,
          "Transporter Name": transporter.transporter_name,
          "Address": transporter.transporter_address,
          ...
          "Created At": new Date(transporter.CreatedAt).toLocaleString("id-ID"),
        }));
        ...
        const colWidths = [
          { wch: 5 },  // No
          { wch: 18 }, // Transporter Code
          { wch: 30 }, // Transporter Name
          { wch: 35 }, // Address
          { wch: 15 }, // City
          { wch: 20 }, // PIC
          { wch: 15 }, // Phone
        ...
        utils.book_append_sheet(wb, ws, "Transporters");

        const timestamp = new Date().toISOString().slice(0, 19).replace(/:/g, "-");
        const filename = `Transporters_Export_${timestamp}.xlsx`;

        writeFile(wb, filename);

        toast.success(`Successfully exported ${transporters.length} transporters`);
      }
    } catch (error) {
      console.error("Failed to export transporters:", error);
