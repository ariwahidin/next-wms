...
const HandleDelete = (id: number) => {
  try {
    api.delete(`/warehouses/${id}`, { withCredentials: true }).then((res) => {
      if (res.data.success === true) {
        mutate("/warehouses"); // 🔥 Auto-refresh tabel tanpa reload halaman
      }
    });
  } catch (error) {
...
const Table = ({ setEditData }) => {
  const { data: rowData, error, mutate } = useSWR("/warehouses", fetcher);

  const [columnDefs, setColumnDefs] = useState<ColDef[]>([
    { field: "no", headerName: "No. ", maxWidth: 70 },
    { field: "code", headerName: "Warehouse Code", width: 140 },
