...
const HandleDelete = (id: number) => {
  try {
    api
      .delete(`/handling/items/${id}`, { withCredentials: true })
      .then((res) => {
        if (res.data.success === true) {
          mutate("/handling/items"); // 🔥 Auto-refresh tabel tanpa reload halaman
        }
      });
  } catch (error) {
...
const ProductTable = ({ setEditData }) => {
  const { data: rowData, error, mutate } = useSWR("/handling/items", fetcher);
  ...
  const [columnDefs, setColumnDefs] = useState<ColDef[]>([
    ...
    { field: "area", headerName: "Area" },
    {
      ...
      width: 200,
      valueGetter: (params) => {
        return params.data.details?.map((d) => d.Handling).join(", ") || "-";
      },
    },
  ...
  const handleEdit = (id: number) => {
    // fetch item by id
    api.get(`/handling/items/${id}`, { withCredentials: true }).then((res) => {
      if (res.data.success) {
