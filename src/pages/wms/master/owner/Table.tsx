...
const HandleDelete = (id: number) => {
  try {
    api.delete(`/owners/${id}`, { withCredentials: true }).then((res) => {
      if (res.data.success === true) {
        mutate("/owners"); // 🔥 Auto-refresh tabel tanpa reload halaman
      }
    });
  } catch (error) {
...
const Table = ({ setEditData }) => {
  const { data: rowData, error, mutate } = useSWR("/owners", fetcher);

  const [columnDefs, setColumnDefs] = useState<ColDef[]>([
    { field: "no", headerName: "No. ", maxWidth: 70 },
    { field: "code", headerName: "Owner Code", width: 140 },
