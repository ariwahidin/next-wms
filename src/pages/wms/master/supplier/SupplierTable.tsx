...
const SupplierTable = () => {
  const { data: rowData, error, mutate: revalidate } = useSWR("/suppliers", fetcher);
  ...
  const handleDelete = useCallback(async (id: number) => {
    ...
    try {
      const res = await api.delete(`/suppliers/${id}`, { withCredentials: true });
  ...
  const [columnDefs] = useState<ColDef[]>([
    ...
    { field: "supplier_name", headerName: "Name", width: 300 },
    {
      ...
      cellRenderer: (params: any) => {
        const addr = params.data.supp_addr1 || "";
        const addr2 = params.data.supp_addr2 || "";
        return `${addr} ${addr2}`.trim();
      },
    },
    { field: "supp_city", headerName: "City" },
    { field: "supp_country", headerName: "Country", maxWidth: 120 },
    { field: "supp_phone", headerName: "Phone", maxWidth: 120 },
  ...
  // Client-side filter by Owner (backend /suppliers belum ada filter query params)
        ...
        <div className="justify-self-start">
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              onClick={() => {
                router.push("/wms/master/supplier/import-excel");
              }}
            >
