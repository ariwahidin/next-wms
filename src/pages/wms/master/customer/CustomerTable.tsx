...
const HandleDelete = (id: number) => {
  try {
    api.delete(`/customers/${id}`, { withCredentials: true }).then((res) => {
...
const CustomerTable = ({ setEditData }) => {
  const { data: rowData, error, mutate } = useSWR("/customers", fetcher);
      ...
      <div className="flex justify-between items-center">
        <div className="justify-self-start">
          <div className="flex items-center">
            <Button className="left-6 h-8 bg-green-500 mb-4 text-slate-950 outline-green-600" onClick={() => { router.push('/wms/master/customer/import-excel') }}>
