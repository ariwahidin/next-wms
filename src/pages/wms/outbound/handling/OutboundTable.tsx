...
import BillModal from "./edit/BillModal";
...
const HandleEdit = (item: any) => {
  ...
  window.location.href = `/wms/outbound/handling/edit/${item.outbound_no}`;
};
...
const OutboundTable = () => {
  const {
    ...
    error,
    mutate,
  } = useSWR("/outbound/handling", fetcher);
      ...
      cellRenderer: (params) => {
        return (
          <div className="flex justify-center space-x-1 pt-2">
            ...
            <Button
              title="View Bill"
              onClick={() => viewBill(params.data.outbound_no)}
              variant="ghost"
              size="icon"
  ...
  const viewBill = async (outbound_no) => {
    try {
      const res = await api.get(`/outbound/handling/bill/${outbound_no}`, {
        withCredentials: true,
      });
      if (res.data.success) {
