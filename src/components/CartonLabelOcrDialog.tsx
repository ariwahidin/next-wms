...
export default function CartonLabelOcrDialog({
  ...
  // Dialog kamera custom (live preview + framing guide garis putus-putus).
  ...
  /**
   * Proses File terpilih (dari sumber manapun: input Gallery,
   * input Camera native, ATAU hasil capture dari CartonCameraCapture).
   *
   * 1. tampilkan preview
   * 2. tunggu 1 detik
  ...
  return (
    ...
    >
      <DialogContent className="max-w-md max-h-[92vh] overflow-y-auto bg-slate-50 p-0 gap-0">
          ...
          {!selectedFile && (
            <div className="rounded-lg border border-dashed border-gray-300 bg-white p-5">
              <div className="flex flex-col items-center text-center">
                ...
                {/* Input kamera native dibiarkan tetap ada (tersembunyi)
                    sebagai fallback kalau suatu saat dibutuhkan lagi;
                    tombol Camera di atas sudah diarahkan ke
                    CartonCameraCapture (custom camera + framing guide). */}
                <input
                  ref={cameraInputRef}
                  type="file"
                  accept="image/*"
        ...
        {/* ── Custom camera (live preview + framing guide) ── */}
        <CartonCameraCapture
